...
-- Create a frame for delayed detection
...
initFrame:SetScript("OnEvent", function()
    ...
    -- Wait briefly after login (2 seconds should be enough)
    ...
    timer:SetScript("OnUpdate", function()
        ...
        -- Check if EngBags is loaded using multiple methods
        local isLoaded = false
        
        -- Check for specific EngBags files (found in your folder)
        if IsAddOnLoaded("EngBags") then
            isLoaded = true
        end
        
        -- Check for specific global objects from your /vpenginv scan
        if getglobal("EngInventory_frame") ~= nil or
           getglobal("EngBags_BUTTONFONTHEIGHT") ~= nil or
           getglobal("EngInventory_item_cache") ~= nil then
            isLoaded = true
        end
        ...
        if not isLoaded then
            if vPeddlerDB and vPeddlerDB.debug then
                DEFAULT_CHAT_FRAME:AddMessage("|cFF99CC33vPeddler|r: EngBags addon not detected, module not loaded")
            end
            return
        end
        ...
        if vPeddlerDB and vPeddlerDB.verboseMode then
            DEFAULT_CHAT_FRAME:AddMessage("|cFF99CC33vPeddler|r: EngBags integration loaded")
        end
        
        -- Continue with the rest of your module's code
        ...
        -- Forward declarations for functions that need to be accessed across scopes
        ...
        UpdateAllIcons = function(forceRecreate)
